...
def add_event(title, location, user_id):
    sql = "INSERT INTO events (title, location, user_id) VALUES (?, ?, ?)"
    db.execute(sql, [title, location, user_id])
    event_id = db.last_insert_id()
    return event_id

def get_event(event_id):
    sql = "SELECT id, title, location, user_id FROM events WHERE id = ?"
    result = db.query(sql, [event_id])
    return result[0] if result else None

def get_events():
    sql = "SELECT id, title, location, user_id FROM events"
    result = db.query(sql)
    return result

def get_events_by_organizer(user_id):
    sql = "SELECT id, title, location, user_id FROM events WHERE user_id = ?"
    result = db.query(sql, [user_id])
    return result

def search(query):
    sql = """SELECT id, title, location, user_id
             FROM events
             WHERE title LIKE ?"""
